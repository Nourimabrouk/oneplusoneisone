# Critical Fix 1: Remove duplicate imports and optimize import order
...
from PyQt5.QtWidgets import (
    QApplication, QWidget, QLabel, QVBoxLayout, QScrollArea, QFrame,
    QGraphicsView, QGraphicsScene, QLineEdit, QPushButton, QHBoxLayout,
    QTabWidget, QTextEdit, QComboBox, QDoubleSpinBox, QGraphicsItem
)
...
class SessionDataHandler:
    ...
    def _serialize_data(self, data: Any) -> Any:
        try:
            ...
            if hasattr(data, 'to_dict'):  # Handle pandas and other convertible objects
                return data.to_dict()
            return data
...
# --- UI Update Thread ---
class OptimizedUIUpdateThread(QThread):
    update_signal = pyqtSignal(dict)
    
    def __init__(self, metastation: 'MetaStation'):
        super().__init__()
...
class Scenario:
    ...
    def run(self):
        ...
        self.env.run()
        return self.results
# Critical Fix 3: Add memory management and resource cleanup
...
class MetaMirrorUI(QWidget):
    def __init__(self):
        super().__init__()
        self.metastation = None  # Will be set after initialization
        self.init_ui()
...
# --- MetaStation Core ---
class AsyncMetaStation(QObject):
    def __init__(self):
        super().__init__()
        self.thread_pool = QThreadPool()
        self.thread_pool.setMaxThreadCount(
            min(32, os.cpu_count() * 2)  # Optimal thread count
        )
